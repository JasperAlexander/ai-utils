...
async function getItems(folder_id: string) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE}/api/folders/${folder_id}/items`,
    {
      cache: 'no-store',
    }
  )
...
export default async function FolderPage({
  ...
  const items: ItemType[] = await getItems(params.folderId)
              ...
              <tr key={item._id} className={styles.tableBodyRow}>
                <td className={styles.tableBodyRowData}>
                  <div className={styles.tableBodyRowDataContent}>
                    ...
                    <Link
                      href={`/${params.userUsername}/${params.projectName}/${params.folderId}/${item._id}`}
                ...
                <td className={styles.tableBodyRowData}>
                  <div className={styles.tableBodyRowDataContent}>
                    <span className={styles.tableBodyRowDataContentSpan}>
                      {item.updated_at &&
                        relativeTimeFormat(new Date(item.updated_at))}
                    </span>
                  </div>
                </td>
                <Suspense>
