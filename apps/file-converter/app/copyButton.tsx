'use client'

import styles from './page.module.css'
import { useCallback, useEffect, useState } from 'react'
import { Tooltip } from 'react-tooltip'
import 'react-tooltip/dist/react-tooltip.css'

export function CopyButton({ filesText }: { filesText: string }) {
  const [copiedFilesText, setCopiedFilesText] = useState(false)

  const copyFilesText = useCallback(() => {
    if (!filesText) return
    navigator.clipboard.writeText(filesText)
    setCopiedFilesText(true)
  }, [filesText])

  useEffect(() => {
    if (!copiedFilesText) return
    const timer = setTimeout(() => {
      setCopiedFilesText(false)
    }, 1500)
    return () => clearTimeout(timer)
  }, [copiedFilesText])

  return (
    <button
      type='button'
      onClick={() => copyFilesText()}
      className={`
        ${styles.textHeaderButton}
        ${
          copiedFilesText ? styles.copyButtonActive : styles.copyButtonInactive
        }`}
      data-tooltip-id='copy-tooltip'
    >
      {copiedFilesText ? (
        <svg
          aria-hidden='true'
          viewBox='0 0 16 16'
          className={styles.textHeaderButtonSvg}
        >
          <path d='M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z'></path>
        </svg>
      ) : (
        <svg
          aria-hidden='true'
          viewBox='0 0 16 16'
          className={styles.textHeaderButtonSvg}
        >
          <path d='M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 0 1 0 1.5h-1.5a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-1.5a.75.75 0 0 1 1.5 0v1.5A1.75 1.75 0 0 1 9.25 16h-7.5A1.75 1.75 0 0 1 0 14.25Z'></path>
          <path d='M5 1.75C5 .784 5.784 0 6.75 0h7.5C15.216 0 16 .784 16 1.75v7.5A1.75 1.75 0 0 1 14.25 11h-7.5A1.75 1.75 0 0 1 5 9.25Zm1.75-.25a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-7.5a.25.25 0 0 0-.25-.25Z'></path>
        </svg>
      )}
      <Tooltip
        id='copy-tooltip'
        content={copiedFilesText ? 'Copied!' : 'Copy plain text'}
        place='bottom'
        className={styles.tooltip}
      />
    </button>
  )
}
